...
server <- function(input, output, session) {# function to caracterize input and output parameters /  session is an environment that can be used to access information and functionality relating to curent session
    ...
    #output$DataTable <- renderDataTable ({#Output a DataTable with renDataTable function of the variable choosen ---- I don't know if it's so important
       # head(inFile()[,input$variable])#Determine the variable choosen in the head of your csv file ----
        
    #})
   
    
    output$dist <- renderPlot({#Output a plot with renderPlot function of the variable choosen
        ...
        if (input$gv) {#If you check grouping variable so the plot is...
                g<-ggplot(inFile(),aes_string(x=input$Variable)) + #aes_string is to output a character type 
                    ...
                    ggtitle("Histogram and Density Estimation of variable into grouping variable ")#Title of plot
    ...
    output$qqplot <- renderPlot({#Output a plot with renderPlot function of the variable choosen
